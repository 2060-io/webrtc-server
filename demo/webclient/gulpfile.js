...
function bundle(options) {
  ...
  console.log(`**** ENVIROMENTS = ${JSON.stringify(process.env)}`)
  ...
  let bundler = browserify({
    ...
    .transform(
      envify({
        ...
        MEDIASOUP_CLIENT_ICESERVER_HOST: process.env.MEDIASOUP_CLIENT_ICESERVER_HOST,
        MEDIASOUP_CLIENT_ICESERVER_PORT: process.env.MEDIASOUP_CLIENT_ICESERVER_PORT,
        ...
        MEDIASOUP_CLIENT_ICESERVER_PROTO: process.env.MEDIASOUP_CLIENT_ICESERVER_PROTO,
        MEDIASOUP_CLIENT_PROTOOPORT: process.env.MEDIASOUP_CLIENT_PROTOOPORT,
        MEDIASOUP_INGRESS_HOST: process.env.MEDIASOUP_INGRESS_HOST,
...
gulp.task('html', () => {
  return gulp.src('index.html').pipe(gulp.dest(OUTPUT_DIR))
})
...
gulp.task('browser:base', gulp.series('clean', 'lint', 'bundle:watch', 'html', 'css', 'resources', 'watch'))

gulp.task(
  'live',
  gulp.series('browser:base', (done) => {
...
gulp.task(
  'devel',
  gulp.series('browser:base', async (done) => {
    const config = require('./config')
      ...
      browserSync.create('producer1').init(
        {
          ...
          host: config.domain,
          startPath:
            '/?roomId=devel:vp9&info=true&_throttleSecret=foo&forceVP9=true&numSimulcastStreams=3&webcamScalabilityMode=L1T3&consume=false',
          server: {
            baseDir: OUTPUT_DIR,
          },
          https: config.https.tls,
...
//Cambios definicion de variables ICE

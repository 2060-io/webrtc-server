...
export default class RoomClient {
  ...
  constructor({
    ...
    // domain to connect urlFActory
    // @type {String}
    this._domain = domain

    // port to connect urlFActory
    // @type {String}
    this._protooPort = protooPort
    ...
    // Protoo URL.
    // @type {String}
    this._protooUrl = getProtooUrl({ roomId, peerId, consumerReplicas, domain, protooPort })
      ...
      switch (request.method) {
        case 'newConsumer': {
          ...
          try {
            ...
            if (consumer.kind === 'video' && store.getState().me.audioOnly) this._pauseConsumer(consumer)
          } catch (error) {
  ...
  async applyNetworkThrottle({ uplink, downlink, rtt, secret, packetLoss }) {
    ...
    try {
      await this._protoo.request('applyNetworkThrottle', { secret, uplink, downlink, rtt, packetLoss })
    } catch (error) {
  ...
  async _joinRoom() {
    ...
    try {
      ...
      store.dispatch(stateActions.setRoomMediasoupClientHandler(this._mediasoupDevice.handlerName))
      ...
      if (this._produce) {
        ...
        if (iceServers) {
          ...
          logger.debug('Turn recvTransport IceServers not defined')
          this._sendTransport = this._mediasoupDevice.createSendTransport({
            id,
            iceParameters,
            iceCandidates,

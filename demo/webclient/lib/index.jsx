...
async function run() {
  ...
  logger.debug('run() [Enable ICE:%s]', process.env.MEDIASOUP_CLIENT_ENABLE_ICESERVER)
  ...
  // add config to set urlFactory
  const domain = urlParser.query.domain
  const protooPort = urlParser.query.protooPort

  // Try to add turn config
  const enableIceServer = process.env.MEDIASOUP_CLIENT_ENABLE_ICESERVER || urlParser.query.enableIceServer
  const iceServerHost = process.env.MEDIASOUP_CLIENT_ICESERVER_HOST || urlParser.query.iceServerHost
  const iceServerProto = process.env.MEDIASOUP_CLIENT_ICESERVER_PROTO || urlParser.query.iceServerProto
  const iceServerPort = process.env.MEDIASOUP_CLIENT_ICESERVER_PORT || urlParser.query.iceServerPort
  const iceServerUser = process.env.MEDIASOUP_CLIENT_ICESERVER_USER || urlParser.query.iceServerUser
  const iceServerPass = process.env.MEDIASOUP_CLIENT_ICESERVER_PASS || urlParser.query.iceServerPass
  // End of Turn Config

  const sharingScalabilityMode = urlParser.query.sharingScalabilityMode
  const numSimulcastStreams = urlParser.query.numSimulcastStreams ? Number(urlParser.query.numSimulcastStreams) : 3

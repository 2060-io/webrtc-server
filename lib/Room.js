...
const { sendNotification } = require('./handlerNotification');
...
class Room extends EventEmitter {
   ...
   */
  static async create({ mediasoupWorker, roomId, consumerReplicas, maxPeerCount }) {
  ...
  constructor({
    ...
    // Max number of peers allowed in the room.
    this._maxPeerCount = maxPeerCount || 2;  // Default to 2 if not provided
   ...
   * @param {eventNotificationUri} - Uri to send notification webhook when peer close
   */
  handleProtooConnection({ peerId, consume, protooWebSocketTransport, eventNotificationUri }) {
   
    // Check if the room has reached the maxPeerCount limit
    if (this._protooRoom.peers.length >= this._maxPeerCount) {
      logger.warn('Room is full, rejecting new peer [roomId:%s, peerId:%s]', this._roomId, peerId);
      protooWebSocketTransport.close();  // Close the connection if the room is full
      return;
    }
    ...
    peer.on('close', () => {
      ...
      sendNotification(eventNotificationUri, joinNotificationData);
  ...
  async _handleProtooRequest(peer, request, accept, reject) {
    switch (request.method) {
      ...
      case 'join': {
        ...
        const joinedPeers = [...this._getJoinedPeers(), ...this._broadcasters.values()]
      ...
      case 'leaveCall': {
        ...
        if (!peer.data.joined) {
          reject(400, 'Peer not joined');
          return;
        }

        // Notify all other peers in the room that this peer has leave the call
        for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })) {
          otherPeer.notify('peerLeaveCall', { peerId: peer.id }).catch(() => {});
        }
        ...
        if (this._protooRoom.peers.length === 0) {
          logger.info('No more peers in room, closing [roomId:%s]', this._roomId);
          this.close();
        }
  ...
  async _createDataConsumer({
    ...
    // Must take the Transport the remote Peer is using for consuming.
    const transport = Array.from(dataConsumerPeer.data.transports.values()).find((t) => t.appData.consuming)

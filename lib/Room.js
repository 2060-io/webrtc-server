...
class Room extends EventEmitter {
   ...
   */
  static async create({ mediasoupWorker, roomId, consumerReplicas, maxPeersRoom }) {
  ...
  constructor({
    ...
    this._maxPeersRoom = maxPeersRoom || 2;  // Default to 2 if not provided
  ...
  handleProtooConnection({ peerId, consume, protooWebSocketTransport }) {
    ...
    // Check if the room has reached the maxPeersRoom limit
    if (this._protooRoom.peers.length >= this._maxPeersRoom) {

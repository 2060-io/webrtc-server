...
class Room extends EventEmitter {
   ...
   */
  static async create({ mediasoupWorker, roomId, consumerReplicas, maxPeersCount }) {
  ...
  constructor({
    ...
    this._maxPeersCount = maxPeersCount || 2;  // Default to 2 if not provided
  ...
  handleProtooConnection({ peerId, consume, protooWebSocketTransport }) {
    ...
    if (this._protooRoom.peers.length >= this._maxPeersCount) {
